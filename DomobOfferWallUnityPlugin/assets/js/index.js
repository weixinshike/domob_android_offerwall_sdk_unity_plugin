...
var scroll_list, __scale = 1;
function __showPage() {
	setTimeout((function(){
		scroll_list && scroll_list.destroy && scroll_list.destroy();

		scroll_list = new iScroll('game_scroller', {"bounce":false});
		var scroll_help = new iScroll('help_scroller', {"bounce":false});
	}), 100);
}
...
function __sdk (functionName, data) {
	...
	DEV?console.log(req):(window.location = req);
}

var __hasBack = false;
function back(){
	__hasBack ? $('.ac_back2list:visible').click() : __sdk('close');
	__hasBack = false;
...
$(function(){
	...
	setTimeout((function(){
		...
		// document.style.webkitTransform = 'scale('+__scale+')';
		document.body.style.zoom = __scale;
	...
	var __clickTime=1;
	$('body')
		.on('click', '.ac_back2list', function(){
			var $box = $(this).closest('.box');
			$box.css('marginLeft', 360);
		})
		.on('click', '.ac_do_action', function(){
			var now = new Date();
			if(now - __clickTime < 1000)return;
			__clickTime = now;
			var app_id = $(this).attr('srv');
			window.location.href = $('.link_go',this).attr('srv');
			__removeTags(this);
		})
		.on('click', '.ac_go_detail', function(){
			__openDetail(this);
			__hasBack = true;
			__removeTags(this);
		})
		.on('click', '.ac_refresh', function(){
			__refresh();
		})
		.on('touchend', '#btn_repo', function(){
			// window.sdk.initAppeal('appealCallback');
			__sdk('initAppeal', {'callback':'appealCallback'});
		})
		.on('touchend', '#btn_help', function(){
